...
function headerElement() {
  ...
  function toggleMobileMenu() {
    ...
    if (isMenuOpen) {
      ...
      menuIcon.innerHTML = getXIcon();
    } else {
      mobileNav.classList.add('hidden');
      menuIcon.innerHTML = getMenuIcon();
    }
  }
    ...
    <div class="container mx-auto px-4 lg:px-8 max-w-[1280px]">
      <div class="flex items-center justify-between h-16">
        ...
        <nav class="hidden md:flex items-center space-x-8">
          ...
          <button onclick="navigateTo('/produtos')" class="text-gray-900 hover:text-blue-600 transition-colors">Produtos</button>
          ...
          <button onclick="navigateTo('/contato')"" class="text-gray-900 hover:text-blue-600 transition-colors">Contato</button>
        </nav>
        ...
        <div class="hidden md:block">
          <button
            ...
            onclick="navigateTo('/agendarconsulta')"
          >
            Agendar Consulta
          </button>
        </div>
      ...
      <div id="mobileNav" class="md:hidden hidden py-4 space-y-4 border-t border-gray-200">
        ...
        <button onclick="scrollToSection('/contato')" class="block w-full text-left text-gray-900 hover:text-blue-600 transition-colors py-2">Contato</button>
        <button onclick="navigateTo('/agendarconsulta')" class="w-full px-6 py-2 bg-gradient-hero text-white font-medium rounded-lg hover:opacity-90 transition-opacity mt-2" style="background: linear-gradient(135deg, #3b82f6 0%, #8b5cf6 100%);">
          Agendar Consulta
        </button>
      </div>
    </div>
